...
import { HomeOutlined, LayoutOutlined, MessageOutlined, MenuFoldOutlined, MenuUnfoldOutlined } from '@ant-design/icons'

const menuItems = [
    { key: '/home', icon: <HomeOutlined />, label: '首页' },
    { key: '/canvas', icon: <LayoutOutlined />, label: '画布' },
    { key: '/charts', icon: <LayoutOutlined />, label: '图形' },
    ...
    return (
        <div className={`flex h-screen ${isSide ? 'flex-row' : 'flex-col'}`}>
            ...
            <div className="flex-1 overflow-hidden flex flex-col">
                {isSide && header}
                <main className="flex-1 overflow-hidden flex flex-col" style={{ position: 'relative' } as React.CSSProperties}>
                    <Outlet />
                </main>
            </div>
        </div>
